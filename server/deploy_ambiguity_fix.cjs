...
const baseLocal = 'd:/VENDA/IZAQUE CAMPESTRE/Saas-Restaurante/server/direct-order';
const baseRemote = '/root/killsis/SAAS-WEB/server/direct-order';

const filesToUpload = [
    { local: 'config.js', remote: 'config.js' },
    { local: 'core/state-machine.js', remote: 'core/state-machine.js' },
    { local: 'core/word-analyzer.js', remote: 'core/word-analyzer.js' }
];

conn.on('ready', () => {
    console.log('SSH connection established for Ambiguity Fix Deployment.');
    conn.sftp((err, sftp) => {
        if (err) {
        ...
        const uploadNext = () => {
            if (uploaded >= filesToUpload.length) {
                console.log('All ambiguity fix files uploaded. Restarting saas-web...');
